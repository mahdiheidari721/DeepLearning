...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        dw = torch.matmul(x.view(x.size(0), -1).t(), dout)
...
class ReLU(object):
    ...
    def backward(dout, cache):
        ...
        """
        x = cache
        dx = torch.where(cache > 0, dout, torch.tensor(0.))
        return dx
...
class Linear_ReLU(object):
    ...
    def forward(x, w, b):
        ...
        out ,relu_cache = ReLU.forward(fc_out)
    ...
    def backward(dout, cache):
        ...
        x = relu_cache
        drelu = torch.where(x > 0, dout, torch.tensor(0.))
        x, w, b = linear_cache
        dx = torch.matmul(drelu, w.t())
        dx = dx.view(x.size())
        dw = torch.matmul(x.view(x.size(0), -1).t(), drelu)
...
def softmax_loss(x, y):
    ...
    N = x.size(0)
    exp_scores = torch.exp(x - x.max(dim=1, keepdim=True).values)
    probs = exp_scores / exp_scores.sum(dim=1, keepdim=True)
    loss = -torch.log(probs[range(N), y]).sum() / N
    dx = probs.clone()
    dx[range(N), y] -= 1
    dx /= N
    return loss, dx
...
class FullyConnectedNet(object):
  ...
  def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
    ...
    for i_layer in range(self.num_layers):
        w_i = torch.normal(0.0, weight_scale, dtype = dtype,size = (layers_dims[i_layer], layers_dims[i_layer+1])).to(device)
...
def create_solver_instance1(data_dict, dtype, device):
     model = TwoLayerNet(hidden_dim=150, dtype=dtype, device=device)
     solver=None
    ###################################################################
    # Use a Solver instance to train a TwoLayerNet that achieves at  #
    ...
    # Create a Solver instance
     solver = Solver(model,
                    data_dict,
                    update_rule=sgd,
                    optim_config={'learning_rate': 0.02},
                    lr_decay=0.99,
                    num_epochs=100,
                    batch_size=160,
                    print_every=100,
...
def sgd_momentum(w, dw, config=None):
    ...
    next_w = None
    ##################################################################
    # Implement the momentum update formula. Store the updated value #
    # in the next_w variable. You should also use and update the      #
    # velocity v.                                                     #
    ##################################################################
...
class Dropout(object):
    ...
    def forward(x, dropout_param):
        ...
        if mode == 'train':
            ...
            ##############################################################
            # Generate a binary mask for dropout
            mask = (torch.rand_like(x) < 1-p).float() / (1 - p)
            ...
            ##############################################################
            # During testing, just return the input as is
            out = x
            ##############################################################
            #                      END OF YOUR CODE                      #
            ##############################################################
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            # The derivative of the dropout mask is 1 for the elements that were kept (mask = 1/p)
            # and 0 for the elements that were dropped (mask = 0).
            dx = dout * mask
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
  ...
  def loss(self, X, y=None):
    ...
    for i_layer in reversed(range(self.num_layers)):
        if i_layer == self.num_layers - 1:
            ...
            if self.use_dropout:
                cache, cache_drop = caches[i_layer]
